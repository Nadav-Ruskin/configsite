...
ENV DEFAULT_DOCKCROSS_IMAGE configsite-debian-arm32v7
COPY dockerfiles/*.deb dockerfiles/configsite-debian-arm32v7.dockerfile /temp/
WORKDIR /temp
RUN dpkg -i configsite_0.1-1_armhf.deb && echo "Configsite installed" || echo "Configsite not installed"
